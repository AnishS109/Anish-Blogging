...
const UpdatePost = () => {
  ...
  const {updatePostTitle} = useContext(DataContext)
  ...
  useEffect(() => {
    const fetchDetailPost = async () => {
      try {
        const response = await fetch(`https://anish-blogging-backend.onrender.com/detail/post/${updatePostTitle}`);
  ...
  const savePost = async () => {
    if (post.username === "") {
      setErrorMsg("Username not found, please Login again to create a post");
      return setDialogOpen(true);
    }
  
    try {
      const response = await fetch(`https://anish-blogging-backend.onrender.com/update/update-post/${updatePostTitle}`, {
        method: "PUT",
      ...
      if (response.ok) {
        ...
        setErrorMsg(data.message || "An error occurred while updating the post.");
        setDialogOpen(true);
      }
    } catch (error) {
